...
template<class T> inline void PatPixelTracking::AddToVector(T element, SharedVec & vec){
	char* t;
	t = (char*)&element;
	for(unsigned int i=0; i<sizeof(element); t++, i++){
		vec.push_back(*t);
	}
}

template<class T> void PatPixelTracking::combine(SharedVec & output, std::vector<std::vector<T> > &input, unsigned int length){
		//unsigned int maxHits = m_hitManager->get_max_hits();
		for (unsigned int i = 0; i<length; i++){
			for (typename std::vector<std::vector<T> >::iterator it = input.begin(); it< input.end(); it++){
				if (i >= (it->size()) ){
					AddToVector(0,output);
				}
				else{
					AddToVector(it->at(i),output);
				}
			}
		}
}
void PatPixelTracking::combineAllVectors(SharedVec &outputVector){
	//hits per event
	for ( std::vector<int>::iterator it = m_hitManager->get_hitsPerEvent().begin(); it != m_hitManager->get_hitsPerEvent().end(); ++it ) {
			AddToVector(*it,outputVector);
	}
	//sensors info
	int sens_num = m_hitManager->get_sensor_hitStartPos()[0].size();
	combine(outputVector,m_hitManager->get_sensor_hitStartPos(),sens_num);
	combine(outputVector,m_hitManager->get_sensor_hitsNum(),sens_num);
	combine(outputVector,m_hitManager->get_sensor_Z(),sens_num);
	//hits
	combine(outputVector,m_hitManager->get_hit_id(),m_hitManager->get_max_hits());
	combine(outputVector,m_hitManager->get_hit_trackId(),m_hitManager->get_max_hits());
	combine(outputVector,m_hitManager->get_hit_sensorNum(),m_hitManager->get_max_hits());
	combine(outputVector,m_hitManager->get_hit_isUsed(),m_hitManager->get_max_hits());
	combine(outputVector,m_hitManager->get_hit_X(),m_hitManager->get_max_hits());
	combine(outputVector,m_hitManager->get_hit_Y(),m_hitManager->get_max_hits());
	combine(outputVector,m_hitManager->get_hit_Z(),m_hitManager->get_max_hits());
	combine(outputVector,m_hitManager->get_hit_W(),m_hitManager->get_max_hits());
	m_hitManager->clear_vectors();
}
...
StatusCode PatPixelTracking::finalize() {
	//eventsNumber * (48*16 + 4 + maxHits*48)
	//TODO: Length as sum of all vectors that we use !
	unsigned int length = 200 * (48*16 + 4 + 4000*48);
	//testProcess(length);
  launchGPU(length);
...
//=============================================================================
//from initialize
void PatPixelTracking::openGPUMemory(){
	using namespace boost::interprocess;
	shared_memory_object shm(open_only,"gpu_memory",read_write);
	try{
		mapped_region region(shm,read_write);
		void * addr       = region.get_address();
		cond = static_cast<shared_conditions*>(addr);
	}
	catch(...){
...
void PatPixelTracking::closeGPUmemory(){
	using namespace boost::interprocess;
	shared_memory_object::remove("gpu_memory");
}
// Can we resize shared memory segment instead of creating a new one every time?
void PatPixelTracking::launchGPU(unsigned int segmentSize){
	using namespace boost::interprocess;
	shared_memory_object::remove("gaudi_memory");
	managed_shared_memory segment
		            (create_only
		            ,"gaudi_memory"
		            ,segmentSize);
	const ShmemAllocator alloc_inst (segment.get_segment_manager());
	m_output_vec = segment.construct<SharedVec>("gaudi_vector")(alloc_inst);
	//receive conditions
	shared_memory_object shm(open_only,"gpu_memory",read_write);
	try{
		mapped_region region(shm,read_write);
		...
		cond = static_cast<shared_conditions*>(addr);
		{
			...
			if(cond->package_in){
				cond->package_full.wait(lock);
			}
			combineAllVectors(*m_output_vec);
		//	int* ttt = (int*)&m_output_vec->at(0);
			cond->package_in = true;
			cond->package_empty.notify_one();
			if(cond->package_in){
				cond->package_full.wait(lock);
			}
			// Do sth with results ! print or sth
		}
	}
	catch(...){
	    shared_memory_object::remove("gaudi_memory");
